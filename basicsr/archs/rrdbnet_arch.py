...
class RRDB_Plus(nn.Module):
    """Residual in Residual Dense Block with Enhanced Features.

    - 추가된 기능:
      ✅ DeepPoolLayer (멀티스케일 Feature Fusion)
      ✅ MultiShapeKernel (Adaptive Filtering)
      ✅ cubic_attention (Local-Global Attention)
    """

    def __init__(self, num_feat, num_grow_ch=32):
        super(RRDB_Plus, self).__init__()
        self.rdb1 = ResidualDenseBlock(num_feat, num_grow_ch)
        self.deep_pool = DeepPoolLayer(num_feat, num_feat)  # ✅ DeepPool 추가
        self.rdb2 = ResidualDenseBlock(num_feat, num_grow_ch)
        self.multi_shape_kernel = MultiShapeKernel(num_feat)  # ✅ MultiShapeKernel 추가
        self.rdb3 = ResidualDenseBlock(num_feat, num_grow_ch)
        self.cubic_att = cubic_attention(num_feat, group=8, dilation=2, kernel=3)  # ✅ Attention 추가

    def forward(self, x):
        out = self.rdb1(x)
        out = self.deep_pool(out)  # ✅ DeepPool 적용
        out = self.rdb2(out)
        out = self.multi_shape_kernel(out)  # ✅ Adaptive Filtering
        out = self.rdb3(out)
        out = self.cubic_att(out)  # ✅ Attention 적용
        return out * 0.2 + x  # ✅ Residual Scaling 유지


@ARCH_REGISTRY.register()
class RRDBNet_Plus(nn.Module):
    """ESRGAN 기반 RRDBNet with Enhanced Blocks"""

    def __init__(self, num_in_ch, num_out_ch, scale=5, num_feat=64, num_block=23, num_grow_ch=32):
        super(RRDBNet_Plus, self).__init__()
        self.scale = scale
        if scale == 2:
            num_in_ch = num_in_ch * 4
        elif scale == 1:
